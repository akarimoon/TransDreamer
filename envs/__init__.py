        ...
        if len(episode_len_) > 0:
            if len(episode_len_) > 1:
                ...
                writer.add_histogram(
                    f"sim/{prefix}/sub_episode_len", episode_len_, env_step
                )
                writer.add_scalar(
                    f"sim/{prefix}/sub_episode_len_min",
                    episode_len_[1:].min(),
                    env_step,
                )
                writer.add_scalar(
                    f"sim/{prefix}/sub_episode_len_max",
                    episode_len_[1:].max(),
                    env_step,
                )
                writer.add_scalar(
                    f"sim/{prefix}/sub_episode_len_mean",
                    episode_len_[1:].mean(),
                    env_step,
                )
                writer.add_scalar(
                    f"sim/{prefix}/sub_episode_len_std",
                    episode_len_[1:].std(),
                    env_step,
                )
