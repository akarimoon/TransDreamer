...
def anneal_learning_rate(global_step, cfg):
    if (global_step - cfg.arch.prefill) < cfg.optimize.warmup_iter:
        ...
        lr = (
            ...
            / cfg.optimize.warmup_iter
            * (global_step - cfg.arch.prefill)
        )
    ...
    # decay
    lr = lr * cfg.optimize.exp_rate ** (
        (global_step - cfg.arch.prefill) / cfg.optimize.decay_step
    )
...
def anneal_temp(global_step, cfg):
    ...
    temp = (
        temp_start
        - (temp_start - temp_end) * (global_step - cfg.arch.prefill) / decay_steps
    )
...
def simulate_test(model, test_env, cfg, global_step, device):
    ...
    with torch.no_grad():
        while not done:
            next_obs, reward, done = test_env.step(
            ...
            action_list, state = model.policy(
                ...
                state,
                training=False,
                context_len=cfg.train.batch_length,
            )
            obs = next_obs
...
def train(model, cfg, device):
    ...
    while global_step < cfg.total_steps:
        with torch.no_grad():
            model.eval()
            next_obs, reward, done = train_env.step(
                action_list[0, -1].detach().cpu().numpy()
            )
            prev_image = torch.tensor(obs[input_type])
            next_image = torch.tensor(next_obs[input_type])
        ...
        if global_step % cfg.train.train_every == 0:
            ...
            model_loss, model_logs, prior_state, post_state = model.world_model_loss(
                global_step, traj, temp
            )
            grad_norm_model = model.world_model.optimize_world_model(
                model_loss, model_optimizer, transformer_optimizer, writer, global_step
            )
            ...
            actor_loss, value_loss, actor_value_logs = model.actor_and_value_loss(
                global_step, post_state, traj, temp
            )
            grad_norm_actor = model.optimize_actor(
                actor_loss, actor_optimizer, writer, global_step
            )
            grad_norm_value = model.optimize_value(
                value_loss, value_optimizer, writer, global_step
            )
