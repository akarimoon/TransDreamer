...
def vis(model, cfg):
    train_dl = get_dataloader(
        cfg.data.dataset, "train", cfg.train.batch_size, cfg.data.data_root
    )
    ...
    checkpointer = Checkpointer(
        os.path.join(cfg.checkpoint.checkpoint_dir, cfg.exp_name),
    ...
    for e in range(start_epoch, cfg.train.epoch):
        for i, (imgs, trj) in enumerate(train_dl):
            ...
            if global_step % cfg.train.print_every_step == 0:
                print(
                    "================Training {} / {}, Seq_T: {}================".format(
                        global_step + 1, total_iter, T
                    )
                )
