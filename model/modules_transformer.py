...
class TransformerDynamic(nn.Module):
    def __init__(self, cfg):
        ...
        if self.q_trans:
            ...
            self.post_stoch_mlp = MLP([d_model, hidden_size, latent_dim_out], act=act, weight_init=weight_init)

        else:
            self.post_stoch_mlp = MLP([q_emb_size, hidden_size, latent_dim_out], act=act, weight_init=weight_init)
...
class ActionDecoder(nn.Module):
    def __init__(self, input_size, action_size, layers, units, dist="onehot", act="relu", min_std=0.1, init_std=5, mean_scale=5, weight_init="xavier"):
        super().__init__()
