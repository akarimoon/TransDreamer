...
class TransDreamer(nn.Module):
    ...
    def compute_world_model_loss(self, traj, optimizer, temp):
        optimizer.zero_grad()
        ...
        return logs, post_state
    
    def compute_actor_and_value_loss(self, traj, post_state, actor_optimizer, value_optimizer, temp):
        actor_optimizer.zero_grad()
        value_optimizer.zero_grad()
        actor_loss, value_loss, logs = self.actor_and_value_loss(traj, post_state, temp)
        actor_loss.backward()
        value_loss.backward()
        if self.grad_clip is not None:
