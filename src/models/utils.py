...
class SubConv2DBlock(nn.Module):
    def __init__(self, c_in, c_out, k, s, p, num_groups, bias=True, non_linearity=True, weight_init="xavier", act="relu"):
        super().__init__()
...
class ConvTranspose2DBlock(nn.Module):
    def __init__(self, c_in, c_out, k, s, p, o_p, num_groups, bias=True, non_linearity=True, weight_init="xavier", act="relu"):
        super().__init__()
...
class ResConv2DBlock(nn.Module):
    def __init__(self, c_in, c_out, num_groups, weight_init="xavier", act="relu"):
        ...
        self.residual = nn.Sequential(
            nn.ReLU(),
            Conv2DBlock(c_in, c_out // 2, 3, 1, 1, num_groups, weight_init=weight_init, act=act),
            Conv2DBlock(c_out // 2, c_out, 1, 1, 0, 0, non_linearity=False, weight_init=weight_init,),
        )

        self.skip = Conv2DBlock(c_in, c_out, 3, 1, 1, num_groups, non_linearity=False,weight_init=weight_init,)
    ...
    def forward(self, inputs):
        return self.non_linear(self.group_norm(self.residual(inputs) + self.skip(inputs)))
...
class ConvLayerNormGRUCell(nn.Module):
    def __init__(self, input_size, hidden_size, init_state=False):
        ...
        self.conv_i2h = Conv2DBlock(input_size, 2 * hidden_size, 3, 1, 1, 0, bias=False, non_linearity=False)  # we have layernorm, bias is redundant
        self.conv_h2h = Conv2DBlock(hidden_size, 2 * hidden_size, 3, 1, 1, 0, bias=False, non_linearity=False)
        self.conv_i2c = Conv2DBlock(input_size, hidden_size, 3, 1, 1, 0, bias=False, non_linearity=False)
        self.conv_h2c = Conv2DBlock(hidden_size, hidden_size, 3, 1, 1, 0, bias=False, non_linearity=False)
...
class ConvLSTMCell(nn.Module):
    def __init__(self, cfg, input_size, hidden_size, spatial_size, k=3, p=1):
        ...
        self.conv_ = Conv2DBlock(input_size + hidden_size, 4 * hidden_size, k, 1, p, 0, bias=False, non_linearity=False, weight_init=weight_init)
...
class ConvLayerNormLSTMCell(nn.Module):
    def __init__(self, cfg, input_size, hidden_size, spatial_size, k=3, p=1):
        ...
        self.conv_i2h = Conv2DBlock(input_size, 4 * hidden_size, k, 1, p, 0, bias=False, non_linearity=False, weight_init=weight_init)
        self.conv_h2h = Conv2DBlock(hidden_size, 4 * hidden_size, k, 1, p, 0, bias=False, non_linearity=False, weight_init=weight_init)
...
class GroupLSTMCell(nn.Module):
    def __init__(self, cfg, input_size, hidden_size, num_units):
        ...
        self.h2h = Linear(
            num_units * hidden_size, 4 * num_units * hidden_size, weight_init=cfg.arch.weight_init, bias=False)
...
def up_and_down_linear_schedule(step, start_step, mid_step, end_step, start_value, mid_value, end_value):
...
class GatedCNN(nn.Module):
    def __init__(self, cfg, input_size, hidden_size, spatial_size):
        ...
        weight_init = cfg.arch.weight_init
        self.conv_h = Conv2DBlock(input_size + hidden_size, hidden_size * 2, k, 1, p, 0, non_linearity=False, weight_init=weight_init)
