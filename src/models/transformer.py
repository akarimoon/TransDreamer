...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        w_head_q = w_head_q.view(
            T_q, bsz, self.n_head, self.d_inner
        )  # qlen x bsz x n_head x d_head
        w_head_k = w_head_k.view(
            T_k, bsz, self.n_head, self.d_inner
        )  # qlen x bsz x n_head x d_head
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.mah = MultiheadAttention(config)
        self.pos_ff = PositionwiseFF(config)

        self.gating = config.gating
        if self.gating:
            self.gate1 = GRUGatingMechanism(config.d_model)
            self.gate2 = GRUGatingMechanism(config.d_model)
...
class Transformer(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.deter_type = config.deter_type

        self.pos_embs = PositionalEmbedding(config.d_model)
